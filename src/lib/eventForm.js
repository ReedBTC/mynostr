/**
 * Shared form helpers for the Events composer.
 *
 * The form is the in-memory shape the composer mutates. It's a superset
 * of what the kind 31922/31923 event carries — fields like `allDay`,
 * `startDate`, `startTime`, `endDate`, `endTime` are UI conveniences
 * that get lowered to spec-shaped `start`/`end`/`start_tzid` tags at
 * publish time.
 *
 * Lives in lib/ so the composer, the import handler, and the
...
export function emptyEventForm() {
  return {
    ...
    image:       '',
    hashtags:    '',
    // Stamp at link-time so the banner says which event this draft will
    // replace even after the user edits the working title.
    linkedEventTitle: '',
    // UI-only: picked place coords from LocationAutocomplete. The user
    // explicitly opts into publishing them as a `g` tag via the "Pin
    // map location" checkbox — until they do, these stay client-side
    // and never appear on the published event. Underscore prefix so
    // the round-trip transforms ignore them.
    _pickedLat: null,
    _pickedLon: null,
  }
}

export function isEventFormMeaningful(form) {
...
// ahead of UTC. Reads back the wall-clock the tz would render then
// differences against the assumed wall-clock.
...
// "2026-04-28T19:30" (datetime-local) → unix seconds, interpreted in
// the supplied IANA tz. Returns NaN on bad input. Two-pass refines DST
...
export function eventToForm(ev) {
  ...
  base.geohash     = parsed.geohash || ''
  base.image       = parsed.image || ''
  base.hashtags    = (parsed.hashtags || []).join(' ')
  base.linkedEventTitle = parsed.title || ''
  return base
}
...
/**
 * Round-trip a form through publish-shape → JSON event template, used
 * for export. Returns the unsigned event object the user will save to
 * disk and re-import later.
 */
export function formToEventTemplate(form, { pubkey = '' } = {}) {
  const lowered = formToPublishShape(form)
  const tags = []
  const dTag = lowered.dTag || randomDTag()
  tags.push(['d', dTag])
  ...
  if (lowered.kind === KIND_TIME_EVENT) {
    ...
    if (lowered.endTzid && lowered.endTzid !== lowered.startTzid) tags.push(['end_tzid', lowered.endTzid])
    const startSec = parseInt(lowered.start, 10)
    if (Number.isFinite(startSec)) tags.push(['D', String(Math.floor(startSec / SECONDS_PER_DAY))])
  }
 ...
 * return a form snapshot ready to seed the composer. Mirrors
 * fetchListingForLoader in the marketplace tray.
 *
